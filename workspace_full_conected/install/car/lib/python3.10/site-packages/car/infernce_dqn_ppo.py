...
STATE_SIZE_DQN = 3 + MAX_CANDIDATES * 2  # [odom_x, odom_y, total_entropy] + (2*MAX_CANDIDATES)
...
class NavigationInferenceTrainer(Node):
    def __init__(self):
        ...
        self.last_action_index = None  # Almacena el índice seleccionado

        # Redes PPO y modelo DQN para inferencia
        ...
        # Cargar modelos guardados (ajusta los paths según tus archivos)
        self.actor.load_weights("actor_model_latest.h5")
        self.critic.load_weights("critic_model_latest.h5")
        self.dqn_model.load_weights("dqn_model_latest.h5")
    ...
    def compute_candidate_features(self):
        ...
        for node in self.filtered_nodes.poses:
            if self.occupied_nodes and self.occupied_nodes.poses:
                ...
                clearance = 10.0
            if clearance < 0.5:  # umbral de penalización (puedes ajustar)
                continue
    ...
    def get_global_state(self):
        ...
        return np.array([
            ...
            avg_obs_dist,
            num_obs_norm,
            0.0  # Aquí se podría incluir delta_heading si se tiene esa variable
        ], dtype=np.float32)
    ...
    # ----- Función step (ciclo de inferencia) -----
    def step(self):
        ...
        # Inferencia con PPO para seleccionar candidato
        ...
        action_index_dqn = self.dqn_model(np.array([dqn_state], dtype=np.float32))
        action_index_dqn = int(np.argmax(action_index_dqn.numpy()[0]))

        # Estrategia de fusión: en este ejemplo usamos PPO (se puede combinar de otras formas)
        ...
        self.publish_candidate_marker(candidate_ppo)
        self.publish_nav_point(candidate_ppo)

        # Publicar la meta (goal) seleccionada para la navegación
        goal_msg = PoseArray()
        goal_msg.header.stamp = self.get_clock().now().to_msg()
        goal_msg.header.frame_id = "map"
        goal_msg.poses.append(candidate_ppo)
