...
# ==============  RED CNN + LSTM  ==========================================
def build_policy(num_frontiers=10):
    # Inputs existentes
    g       = tf.keras.Input(shape=(PATCH,PATCH,1), name="grid")
    st      = tf.keras.Input(shape=(4,),            name="state")
    w0      = tf.keras.Input(shape=(2,),            name="wp0")
    # Nuevo input: N fronteras (x,y) relativas
    fronts  = tf.keras.Input(shape=(num_frontiers,2),name="frontiers")
    ...
    # Embedding de las fronteras
    f = tf.keras.layers.Flatten()(fronts)
    f = tf.keras.layers.Dense(64, activation="relu")(f)

    # Mezcla todo
    merged = tf.keras.layers.Concatenate()([z, f])

    # LSTM un paso como antes
    h0 = tf.keras.layers.Dense(128,activation="tanh")(merged)
    c0 = tf.keras.layers.Dense(128,activation="tanh")(merged)
    lstm = tf.keras.layers.LSTMCell(128)
    h1,_ = lstm(w0,[h0,c0])

    # --- Cabezas de salida dual ---
    # 1) Discreta: softmax sobre N fronteras
    logits = tf.keras.layers.Dense(num_frontiers, name="logits")(h1)
    prob_f = tf.keras.layers.Activation("softmax", name="prob_front")(logits)
    # 2) Continua: delta de movimiento
    delta  = tf.keras.layers.Dense(2, activation="tanh", name="delta")(h1)

    return tf.keras.Model([g, st, w0, fronts], [prob_f, delta], name="policy")
...
class FlexPlanner(Node):
    ...
    def frontier_score(self, f, grid, info, gp, w_area=0.5):
        """
        Calcula un coste de una frontera f combinando:
         - distancia euclídea al goal (gp)
         - área accesible alrededor de f
        """
        # 1) distancia al goal
        d_goal = l2(f, gp)
        # 2) estimar área accesible con BFS limitado
        H, W = grid.shape
        i0,j0 = idx_from_world(info, f)
        seen = {(i0,j0)}
        q = deque([(i0,j0)])
        area = 0
        max_cells = 500
        while q and area < max_cells:
            ...
            for di, dj in [(1,0),(-1,0),(0,1),(0,-1)]:
                ni, nj = i+di, j+dj
                if (ni, nj) not in seen and 0<=ni<W and 0<=nj<H:
                    seen.add((ni,nj))
                    if grid[nj,ni] < 100:  # libre o desconocido
                        q.append((ni,nj))
                        area += 1
        # coste: distancia - peso·área
        return d_goal - w_area*area
    ...
    def choose_target(self, cp, grid, info):
            ...
            cands = [f for f in self.frontiers
                    if bres_free(grid,info,cp,f)
                    and clearance_ok(grid,info,f,CLEAR_MIN)]
            if not cands:
                return None, "NONE"

            # selecciona la de menor coste
            scored = [(f, self.frontier_score(f, grid, info, gp)) for f in cands]
            best_f, _ = min(scored, key=lambda x: x[1])
            return best_f, "FRONTIER"
...
###nuevo
    ...
    def step(self):
        ...
        if tgt is None:
            ...
            # 3 · Construir tensor de fronteras relativas ------------------
        rels = []
        for f in self.frontiers[:self.num_frontiers]:
            rels.append([f[0] - cp[0], f[1] - cp[1]])
        while len(rels) < self.num_frontiers:
            rels.append([0.0, 0.0])
        fronts_arr = np.array(rels, dtype=np.float32)[None, ...]  # (1, N, 2)

        # 4 · Preparar inputs para la política ------------------------
        # estado: [vx, vy, dx_goal, dy_goal]
        dxg, dyg = gp[0] - cp[0], gp[1] - cp[1]
        state = np.array([0.0, 0.0, dxg, dyg], dtype=np.float32)[None, ...]
        w0    = np.zeros((1, 2), dtype=np.float32)

        # 5 · Inference y muestreo de frontera ------------------------
        prob_f_b, delta_b = self.policy([patch[None, ...], state, w0, fronts_arr],
                                        training=False)
        prob_f = prob_f_b.numpy()[0]    # (N,)
        delta  = delta_b .numpy()[0]    # (2,)

        # sampleo discreto
        i_sel = np.random.choice(self.num_frontiers, p=prob_f)
        logp_f = np.log(prob_f[i_sel] + 1e-8)

        # 6 · Determinar target: GOAL o frontera seleccionada ----------
        if self.goal_vis >= GOAL_VIS_OK:
            tgt, mode = gp, "GOAL"
        else:
            tgt, mode = self.frontiers[i_sel], "FRONTIER"
        ...
        self.val_buf.append(self.value_net(state_vec[None, ...])[0, 0])
        self.done_buf.append(reached or collided)
        self.act_f_buf.append(i_sel)
        self.logp_f_buf.append(np.float32(logp_f))
        self.rew_buf.append(reward)
    ...
    def update_ppo(self):
        # 1) Cálculo de returns y ventajas (GAE)
        returns, advs = [], []
        gae = 0.0
        next_val = 0.0
        for r, v, d in zip(reversed(self.rew_buf),
        ...
        # 2) Dataset PPO con acciones discretas y continuas
        ds = tf.data.Dataset.from_tensor_slices(
            (
                np.stack(self.patch_buf).astype(np.float32),   # (N, PATCH, PATCH, 1)
                np.stack(self.state_buf).astype(np.float32),   # (N, 4)
                np.stack(self.act_buf).astype(np.float32),     # (N, 2) cont.
                np.array(self.logp_buf, np.float32),           # (N,) cont. old logp
                np.array(self.act_f_buf, np.int32),            # (N,) disc.
                np.array(self.logp_f_buf, np.float32),         # (N,) disc. old logp
                advs.astype(np.float32),                       # (N,)
                returns.astype(np.float32)                     # (N,)
            )
        ).shuffle(4096).batch(BATCH_SZ)

        # 3) Optimización actor-critic
        for _ in range(EPOCHS):
            for (g, st, act_cont, logp_cont_old,
                 act_disc, logp_disc_old, adv, ret) in ds:

                with tf.GradientTape(persistent=True) as tape:
                    # Forward para políticas
                    prob_f_new, mu = self.policy(
                        [g, st, tf.zeros((tf.shape(st)[0],2), tf.float32),
                         tf.zeros((tf.shape(st)[0], self.num_frontiers, 2), tf.float32)],
                        training=True)
                    std = tf.exp(self.log_std)

                    # --- Discrete PPO loss ---
                    # Selecciona probabilidades de las acciones tomadas
                    idxs = tf.stack([tf.range(tf.shape(prob_f_new)[0]), act_disc], axis=1)
                    prob_sel = tf.gather_nd(prob_f_new, idxs)
                    logp_disc_new = tf.math.log(prob_sel + 1e-8)
                    ratio_disc = tf.exp(logp_disc_new - logp_disc_old)
                    pg_loss_disc = -tf.reduce_mean(
                        tf.minimum(ratio_disc * adv,
                                   tf.clip_by_value(ratio_disc, 1-CLIP_EPS, 1+CLIP_EPS) * adv)
                    )

                    # --- Continuous PPO loss ---
                    logp_cont_new = -0.5 * tf.reduce_sum(
                        ((act_cont - mu) / std) ** 2 + 2 * tf.math.log(std) + tf.math.log(2 * np.pi),
                        axis=-1)
                    ratio_cont = tf.exp(logp_cont_new - logp_cont_old)
                    pg_loss_cont = -tf.reduce_mean(
                        tf.minimum(ratio_cont * adv,
                                   tf.clip_by_value(ratio_cont, 1-CLIP_EPS, 1+CLIP_EPS) * adv)
                    )

                    # Total policy loss
                    pg_loss = pg_loss_disc + pg_loss_cont

                    # --- Value loss ---
                    state_vec = tf.concat([
                        tf.reshape(g, (-1, PATCH * PATCH))[:, :32], st
                    ], axis=-1)
                    v = tf.squeeze(self.value_net(state_vec, training=True), axis=-1)
                    v_loss = tf.reduce_mean((ret - v) ** 2)

                # Aplicar gradientes
                vars_actor = self.policy.trainable_variables + [self.log_std]
                grads_actor = tape.gradient(pg_loss, vars_actor)
                self.opt_actor.apply_gradients(zip(grads_actor, vars_actor))

                grads_critic = tape.gradient(v_loss, self.value_net.trainable_variables)
                self.opt_critic.apply_gradients(zip(grads_critic, self.value_net.trainable_variables))
        ...
        # 5) Logs y guardado de pesos
        with self.writer.as_default():
            tf.summary.scalar("loss_actor_disc", pg_loss_disc, step=self.episode)
            tf.summary.scalar("loss_actor_cont", pg_loss_cont, step=self.episode)
            tf.summary.scalar("loss_critic", v_loss, step=self.episode)
            tf.summary.scalar("policy_std", float(new_std[0]), step=self.episode)
        self.policy.save_weights(RUN_DIR / f"policy_latest_{self.ts}.weights.h5")
