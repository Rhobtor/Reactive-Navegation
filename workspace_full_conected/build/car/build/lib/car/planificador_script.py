...
from nav_msgs.msg import Odometry, OccupancyGrid, Path, MapMetaData
from geometry_msgs.msg import PoseArray, Pose, PoseStamped, Twist, Point, Quaternion
...
class NavigationNode(Node):
    def __init__(self):
        ...
        # Parámetros del robot (huella real)
        self.robot_width = 0.8    # ancho del robot (m)
        self.robot_length = 1.2   # largo del robot (m)
    ...
    # ------------------------
    # Funciones para verificación de la huella del robot
    # ------------------------

    def point_in_polygon(self, point, polygon):
        """
        Comprueba si un punto (x, y) se encuentra dentro de un polígono usando el algoritmo Ray Casting.
        """
        x, y = point
        inside = False
        n = len(polygon)
    ...
    def is_footprint_safe(self, fused_grid, info, center, theta, robot_width, robot_length):
        """
        Verifica si la huella (rectangular) del robot es segura en la posición 'center' con orientación 'theta'.
        Se construye el polígono de la huella y se chequean todas las celdas que se encuentren en ese polígono.
        """
        half_length = robot_length / 2.0
        half_width = robot_width / 2.0
        # Definir los vértices del rectángulo (en coordenadas locales)
        local_corners = [
        ...
        # Rotar y trasladar los vértices a coordenadas del mundo
        world_corners = []
        ...
        # Convertir límites a índices del grid
        i_min = int((min_x - ox) / resolution)
        i_max = int((max_x - ox) / resolution) + 1
        j_min = int((min_y - oy) / resolution)
        j_max = int((max_y - oy) / resolution) + 1

        # Verificar cada celda del bounding box
        for j in range(j_min, j_max):
            for i in range(i_min, i_max):
                # Coordenadas del centro de la celda
                cell_x = ox + (i + 0.5) * resolution
                cell_y = oy + (j + 0.5) * resolution
                if self.point_in_polygon((cell_x, cell_y), world_corners):
                    if not (0 <= i < fused_grid.shape[1] and 0 <= j < fused_grid.shape[0]):
                        return False  # Fuera del grid se considera inseguro
                    if fused_grid[j, i] == 100:  # Valor 100 representa obstáculo
                        return False
        return True

    def is_path_clear_with_footprint(self, fused_grid, info, start, end, robot_width, robot_length, step=0.1):
        """
        Chequea que a lo largo de la línea entre start y end, la huella del robot sea segura.
        Se muestrea la línea en intervalos de 'step' (en metros).
        """
        total_dist = math.hypot(end[0] - start[0], end[1] - start[1])
        num_steps = max(int(total_dist / step), 1)
        
        for i in range(num_steps + 1):
            ratio = i / num_steps
            x = start[0] + ratio * (end[0] - start[0])
            y = start[1] + ratio * (end[1] - start[1])
            # Aproximar la orientación según la dirección del segmento
            if i < num_steps:
                x_next = start[0] + ((i + 1) / num_steps) * (end[0] - start[0])
                y_next = start[1] + ((i + 1) / num_steps) * (end[1] - start[1])
                theta = math.atan2(y_next - y, x_next - x)
            else:
                theta = math.atan2(end[1] - start[1], end[0] - start[0])
            if not self.is_footprint_safe(fused_grid, info, (x, y), theta, robot_width, robot_length):
                return False
        return True

    # Función para publicar un marker que muestre la huella del robot en RViz
    def publish_robot_marker(self, position, orientation):
        marker = Marker()
        ...
        marker.pose.orientation = orientation  # Se espera un geometry_msgs/Quaternion
        marker.scale.x = self.robot_length  # Longitud del robot
        marker.scale.y = self.robot_width   # Ancho del robot
        marker.scale.z = 0.1                # Grosor para visualización
        marker.color.a = 0.5
        marker.color.r = 1.0
        marker.color.g = 0.0
        marker.color.b = 0.0
    ...
    # ------------------------
    # Funciones de planificación y verificación
    # ------------------------

    def is_line_free(self, fused_grid, info, start, end):
        """
        Función antigua de comprobación de línea libre (sin considerar la huella completa).
        Se conserva para algunos usos, pero se recomienda usar is_path_clear_with_footprint.
        """
        def world_to_index(point, info):
            i = int((point[0] - info.origin.position.x) / info.resolution)
            j = int((point[1] - info.origin.position.y) / info.resolution)
        ...
        if dx > dy:
            err = dx / 2.0
            while x != x1:
                if not (0 <= x < fused_grid.shape[1] and 0 <= y < fused_grid.shape[0]) or fused_grid[y, x] == 100:
                    return False
                err -= dy
                if err < 0:
                    y += sy
    ...
    def check_effective_path_safety(self, path, fused_grid, info):
        """
        Revisa cada segmento del path para asegurarse de que, a lo largo del mismo, la huella del robot se mantenga en zona segura.
        """
        for i in range(len(path) - 1):
            if not self.is_path_clear_with_footprint(fused_grid, info, path[i], path[i+1],
    ...
    def rrt_plan_path(self, start, goal, fused_grid, info, max_iterations=1000, step_size=1.0, goal_threshold=1.0, goal_bias=0.2):
        ...
        for i in range(max_iterations):
            ...
            new_point = (nearest['point'][0] + step_size * math.cos(theta),
                         nearest['point'][1] + step_size * math.sin(theta))
            # Usamos la verificación con huella
            if not self.is_path_clear_with_footprint(fused_grid, info, nearest['point'], new_point,
                                                      self.robot_width, self.robot_length):
                continue
            new_node = {'point': new_point, 'parent': nearest}
            tree.append(new_node)
            if distance(new_point, goal) < goal_threshold and \
               self.is_path_clear_with_footprint(fused_grid, info, new_point, goal,
                                                  self.robot_width, self.robot_length):
                goal_node = {'point': goal, 'parent': new_node}
                tree.append(goal_node)
                path = []
    ...
    def plan_path(self, nodes, fused_grid, info):
        ...
        for i in range(len(all_nodes)):
            for j in range(i + 1, len(all_nodes)):
                ...
                if d <= self.connection_threshold:
                    # Usamos la verificación con huella
                    if self.is_path_clear_with_footprint(fused_grid, info, all_nodes[i], all_nodes[j],
                                                         self.robot_width, self.robot_length):
                        graph[i].append((j, d))
                        graph[j].append((i, d))
        path_indices = dijkstra(graph, robot_index, goal_index)
        if not path_indices or len(path_indices) < 3:
            ...
            return None  # Se fuerza replan usando otro método
        path = [all_nodes[i] for i in path_indices]
        # Verificar que cada segmento del path sea seguro con la huella
        for i in range(len(path) - 1):
            if not self.is_path_clear_with_footprint(fused_grid, info, path[i], path[i+1],
                                                     self.robot_width, self.robot_length):
                self.get_logger().warn(f"Segmento inseguro entre {path[i]} y {path[i+1]}. Forzando replan.")
    ...
    def fallback_path(self, current_pos, goal_pos, fused_grid, info, step_size=0.2, offset_step=0.3, max_offset=1.0):
        """
        Genera un path incremental entre current_pos y goal_pos en pasos de step_size.
        Si algún segmento no es seguro, se desplaza el punto intermedio en dirección perpendicular.
        """
        path = []
        total_dist = distance(current_pos, goal_pos)
        num_steps = int(total_dist / step_size)
        ...
        for i in range(1, len(path)):
            prev_pt = safe_path[-1]
            pt = path[i]
            if self.is_path_clear_with_footprint(fused_grid, info, prev_pt, pt,
                                                 self.robot_width, self.robot_length):
                safe_path.append(pt)
            else:
                dx = pt[0] - prev_pt[0]
                ...
                for sign in [1, -1]:
                    ...
                    while offset <= max_offset:
                        new_pt = (pt[0] + sign * offset * math.cos(perp_angle),
                                  pt[1] + sign * offset * math.sin(perp_angle))
                        if self.is_path_clear_with_footprint(fused_grid, info, prev_pt, new_pt,
                                                             self.robot_width, self.robot_length):
                            safe_path.append(new_pt)
                            found_safe = True
                            break
    ...
    def control_loop(self):
        ...
        # Publicar el marker de la huella del robot usando la posición actual y orientación
        self.publish_robot_marker(current_pos, self.odom.orientation)
        ...
        # Calcular velocidades usando Pure Pursuit
        q = self.odom.orientation
        siny_cosp = 2.0 * (q.w * q.z + q.x * q.y)
        cosy_cosp = 1.0 - 2.0 * (q.y * q.y + q.z * q.z)
        current_yaw = math.atan2(siny_cosp, cosy_cosp)
    ...
    def compute_dynamic_twist(self, current_pos, current_yaw, path, fused_grid, info):
        ...
        # Comprobar obstáculos locales (se puede mantener la función original)
        safe_distance = 0.5
        obstacle_detected, obs_distance = self.check_local_obstacles(fused_grid, info, current_pos, current_yaw)
        if obstacle_detected and obs_distance < safe_distance:
            self.get_logger().warn(f"Obstáculo muy cerca ({obs_distance:.2f} m), activando retroceso.")
    ...
    def should_replan(self, current_pos):
        # Se puede definir una condición para replanificar globalmente.
        return False
