...
class shekel(object):
    ...
    def read(self, position=None):
        ...
        if position is None:
            ...
            position[:, 0] = np.clip(position[:, 0], 0, self.normalized_z.shape[0] - 1)
            position[:, 1] = np.clip(position[:, 1], 0, self.normalized_z.shape[1] - 1)
...
if __name__ == "__main__":
    ...
    ypacarai_map = np.genfromtxt('map.txt',delimiter=' ',dtype=int)
